...
class RegularizedLinearRegressor_Multi:
    ...
    def train(self,X,y,reg=1e-5,num_iters=10000):
...
class RegularizedLinearReg_SquaredLoss(RegularizedLinearRegressor_Multi):
    ...
    def loss (self,*args):
        ...
        ###########################################################################
        theta1 = theta[1:]
        J = 0.5*np.sum((y-np.dot(X, theta))**2)/num_examples + reg*0.5*np.sum(theta1**2)/num_examples
    ...
    def grad_loss(self,*args):                                                                          
        ...
        ###########################################################################
        theta1 = theta[1:]
        grad = np.dot(X.T, (np.dot(X, theta)-y))/num_examples
        grad[1:] += reg*theta1/num_examples
