...
class LogisticRegressor:
    ...
    def loss(self, *args):
        ...
        J = -1 * (1.0 / m) * (np.dot(y.T, np.log(h)) + np.dot((1 - y).T, np.log(1 - h)))
    ...
    def predict(self, X):
        ...
        # TODO: 1 line of code expected                                           #
        ###########################################################################
        y_pred = (utils.sigmoid(np.dot(X, self.theta)) > 0.5) * 1
...
class RegLogisticRegressor:
    ...
    def loss(self, *args):
        ...
        ##########################################################################
        h = utils.sigmoid(np.dot(X, theta))
        reg_part = reg / (2 * m) * np.sum(np.square(theta[1:]))
        J = -1 * (1.0 / m) * (np.dot(y.T, np.log(h)) + np.dot((1 - y).T, np.log(1 - h))) + reg_part
    ...
    def grad_loss(self, *args):
        ...
        ##########################################################################
        h = utils.sigmoid(np.dot(X, theta))
        grad = (1.0 / m) * np.dot(X.T, h - y) + np.append([0], reg / m * theta[1:])
