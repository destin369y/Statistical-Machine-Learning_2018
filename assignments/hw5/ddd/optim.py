...
def sgd_momentum(theta, dtheta, config=None):
  ...
  - velocity: A numpy array of the same shape as theta and dtheta used to store a moving
  ...
  #############################################################################
  momentum, lr = config['momentum'], config['learning_rate']
  v = momentum * v - lr * dtheta
  next_theta = theta + v
...
def rmsprop(theta, dtheta, config=None):
  ...
  cache = dr * cache + (1 - dr) * (dtheta**2)
  next_theta = theta - lr * dtheta / (np.sqrt(cache) + eps)
  config['cache'] = cache
  pass
  #############################################################################
  #                             END OF YOUR CODE                              #
...
def adam(theta, dtheta, config=None):
  ...
  mb = m / (1 - beta1 ** t)
  vb = v / (1 - beta2 ** t)
  next_theta = theta - lr * mb / (np.sqrt(vb) + eps)
